...
const VideoCard = ({ title, thumbnail, views, duration, aspectRatio = "video", className = "", link }: VideoCardProps) => {
  const aspectClasses = {
    video: "aspect-video",
  ...
  if (link) {
    return (
      <a href={link} target="_blank" rel="noopener noreferrer" className={`group relative rounded-3xl overflow-hidden video-card-hover ${className} block`}>
        {CardContent}
      </a>
    );
  }
